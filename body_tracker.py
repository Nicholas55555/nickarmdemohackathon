"""
Two-Hand Tracker — Forearm for Rotations, Fingers for Joints.

LEFT HAND:
  Forearm horizontal sweep → J1 base rotation
  Middle finger MCP bend   → J2 shoulder (finger knuckle = shoulder)
  Middle finger PIP bend   → J3 elbow   (finger middle joint = elbow)

RIGHT HAND:
  Hand tilt vs horizontal  → J4 wrist pitch
  Palm roll vs horizontal  → J5 wrist rotation
  Thumb vs 4 fingertips    → J6 claw open/close

Your left middle finger IS the robot arm — curl it to move the arm.
"""
...
P_L_ELBOW = 13; P_L_WRIST = 15
P_R_ELBOW = 14; P_R_WRIST = 16
...
def _joint_bend_angle(a, b, c):
    """
    Compute bend angle at joint B given points A, B, C (2D numpy arrays).
    Returns 0° when straight (A→B→C collinear), increases as B flexes.
    """
    v1 = a - b  # incoming
    v2 = c - b  # outgoing
    ...
    return 180.0 - math.degrees(math.acos(cos_a))  # 0=straight, 180=fully folded
...
class HandPairTracker:
    ...
    def __init__(self):
        ...
        # Forearm sensitivity (0-100): controls forearm vs hand for rotation
        self.j1_forearm_sens = 70   # J1: left forearm sweep
        self.j2_forearm_sens = 0    # unused (J2 = finger joint)
        self.j4_forearm_sens = 0    # unused (J4 = finger joint)
        self.j5_forearm_sens = 70   # J5: right forearm sweep

        # Finger sensitivity (0-100): scales finger joint angle contribution
        self.left_finger_sens = 50   # J2/J3 from left middle finger
        self.right_finger_sens = 50  # J4/J5 from right hand gestures
    ...
    # ══════════════════════════════════════════════════════════════════
    # LEFT HAND → J1 (forearm rotation), J2/J3 (finger joints)
    # ══════════════════════════════════════════════════════════════════
    def _extract_position_joints(self, lms, h, w, forearm_dir=None):
        """
        J1: forearm sweep (with wrist-X fallback)
        J2: index finger MCP bend (knuckle = shoulder)
        J3: index finger PIP bend (middle joint = elbow)
        """
        wrist = np.array([lms[WRIST][0], lms[WRIST][1]])
        idx_mcp = np.array([lms[INDEX_MCP][0], lms[INDEX_MCP][1]])
        idx_pip = np.array([lms[INDEX_PIP][0], lms[INDEX_PIP][1]])
        idx_dip = np.array([lms[INDEX_DIP][0], lms[INDEX_DIP][1]])
        mid_mcp = np.array([lms[MIDDLE_MCP][0], lms[MIDDLE_MCP][1]])
        hand_scale = np.linalg.norm(wrist - mid_mcp)

        fmul = max(0.2, self.left_finger_sens / 50.0)
        fa_w = self.j1_forearm_sens / 100.0
        has_fa = forearm_dir is not None and np.linalg.norm(forearm_dir) > 0.01

        # ── J1: forearm sweep + wrist X fallback ─────────────────────
        j1_hand = -(wrist[0] - 0.5) * 2.0 * 90.0
        if has_fa:
            j1_forearm = math.degrees(math.atan2(-forearm_dir[0], forearm_dir[1]))
            j1 = j1_forearm * fa_w + j1_hand * (1 - fa_w)
        else:
            j1 = j1_hand
        j1 = _clamp(j1, *SERVO["J1"][:2])

        # ── J2: index MCP bend (wrist → MCP → PIP) ──────────────────
        mcp_angle = _joint_bend_angle(wrist, idx_mcp, idx_pip)
        j2 = _remap(mcp_angle * fmul, 5, 100 * fmul,
                     SERVO["J2"][0], SERVO["J2"][1])
        j2 = _clamp(j2, *SERVO["J2"][:2])

        # ── J3: index PIP bend (MCP → PIP → DIP) ────────────────────
        pip_angle = _joint_bend_angle(idx_mcp, idx_pip, idx_dip)
        j3 = _remap(pip_angle * fmul, 5, 90 * fmul,
    ...
    # ══════════════════════════════════════════════════════════════════
    # RIGHT HAND → J4 (index finger bend), J5 (forearm rotation), J6 (claw)
    # ══════════════════════════════════════════════════════════════════
    def _extract_claw_joints(self, lms, h, w, forearm_dir=None):
        """
        J4: index finger MCP bend (knuckle curl = wrist pitch)
        J5: right forearm sweep angle (rotation, like J1 uses left forearm)
            Falls back to palm roll vs horizontal if no forearm.
        J6: thumb vs 4 fingertips (claw open/close gesture)
        """
        wrist = np.array([lms[WRIST][0], lms[WRIST][1]])
        idx_mcp = np.array([lms[INDEX_MCP][0], lms[INDEX_MCP][1]])
        idx_pip = np.array([lms[INDEX_PIP][0], lms[INDEX_PIP][1]])
        ...
        fmul = max(0.2, self.right_finger_sens / 50.0)
        j5_w = self.j5_forearm_sens / 100.0
        has_fa = forearm_dir is not None and np.linalg.norm(forearm_dir) > 1e-6

        # ── J4: index finger MCP bend → wrist pitch ──────────────────
        # Bend at knuckle: wrist → index_MCP → index_PIP
        j4_angle = _joint_bend_angle(wrist, idx_mcp, idx_pip)
        # Map: straight (0°) → J4 min, curled (90°+) → J4 max
        j4 = _remap(j4_angle * fmul, 5, 90 * fmul,
                     SERVO["J4"][0], SERVO["J4"][1])
        j4 = _clamp(j4, *SERVO["J4"][:2])

        # ── J5: forearm rotation (with palm-roll fallback) ────────────
        horizontal = np.array([1.0, 0.0])
        palm_vec = pk_mcp - idx_mcp

        j5_hand = (_signed_angle_2d(palm_vec, horizontal) * fmul
                   if np.linalg.norm(palm_vec) > 1e-6 else 0.0)

        if has_fa and j5_w > 0.01:
            # Forearm sweep: horizontal angle of elbow→wrist vector
            j5_forearm = math.degrees(math.atan2(-forearm_dir[0], forearm_dir[1]))
            j5 = j5_forearm * j5_w + j5_hand * (1 - j5_w)
        else:
            j5 = j5_hand
        j5 = _clamp(j5, *SERVO["J5"][:2])

        # ── J6: thumb vs 4 fingertips (claw gesture) ─────────────────
    ...
    def process(self, bgr_frame):
        ...
        if left_hand:
            ...
            # Highlight index finger chain for J2/J3 (yellow)
            l_chain = [WRIST, INDEX_MCP, INDEX_PIP, INDEX_DIP, INDEX_TIP]
            for i in range(len(l_chain)-1):
                cv2.line(frame, pts[l_chain[i]], pts[l_chain[i+1]], (0,255,255), 3)
            for idx in l_chain[1:]:
                cv2.circle(frame, pts[idx], 5, (0,255,255), -1)
            fa_t = "+FA" if l_forearm_dir is not None else ""
            cv2.putText(frame, f"L-FINGER{fa_t}", (pts[0][0]-30, pts[0][1]-15),
        ...
        if right_hand:
            ...
            # Highlight index finger chain for J4 (blue)
            r_chain = [WRIST, INDEX_MCP, INDEX_PIP, INDEX_DIP, INDEX_TIP]
            for i in range(len(r_chain)-1):
                cv2.line(frame, pts[r_chain[i]], pts[r_chain[i+1]], (255,150,0), 3)
            for idx in r_chain[1:]:
                cv2.circle(frame, pts[idx], 5, (255,150,0), -1)
            fa_t = "+FA" if r_forearm_dir is not None and self.j5_forearm_sens > 0 else ""
        ...
        # Show finger joint angles for debugging
        mcp_a = pip_a = 0
        if left_hand:
            wr = np.array([left_hand["landmarks"][WRIST][0], left_hand["landmarks"][WRIST][1]])
            imcp = np.array([left_hand["landmarks"][INDEX_MCP][0], left_hand["landmarks"][INDEX_MCP][1]])
            ipip = np.array([left_hand["landmarks"][INDEX_PIP][0], left_hand["landmarks"][INDEX_PIP][1]])
            idip = np.array([left_hand["landmarks"][INDEX_DIP][0], left_hand["landmarks"][INDEX_DIP][1]])
            mcp_a = _joint_bend_angle(wr, imcp, ipip)
            pip_a = _joint_bend_angle(imcp, ipip, idip)

        info = [f"{lh} {lf} {rh} {rf}",
                f"J1:{smoothed.get('J1',0):+5.0f} MCP:{mcp_a:4.0f} PIP:{pip_a:4.0f}",
                f"J2:{smoothed.get('J2',0):5.0f} J3:{smoothed.get('J3',0):5.0f}",
